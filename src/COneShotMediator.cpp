...
void COneShotMediator::mainLoop()
{
  ...
  while (1) {
    ...
    if (tot_iter>=nskip) {
      ...
      // Update the state of our OneShotHandler... the next line is a hack 
      // that needs to be fixed
      CRingItem* pItem = CRingItemFactory::createRingItem(item->getItemPointer());
      delete item;
      item = pItem;
...
// The following applies to 11.0 code and is not appropriate for 10.2
//      } else {
...
void COneShotMediator::initialize() { getFilter()->initialize(); }
void COneShotMediator::finalize() { getFilter()->finalize(); }
